...
/**
 * When the vendored phishing list was fetched, from the header that
 * update-phishing-list.js writes ("! Generated by ... on <ISO date>").
 *
 * This is the seed's real age, and unlike the build clock it is the same on
 * every build of a commit: Mozilla reviewers rebuild the add-on from the
 * source archive and diff it against the submitted package.
 */
export function phishingListDate(file = 'phishing.txt') {
  const text = readFileSync(join(FILTERS_DIR, file), 'utf-8');
  const match = /^!\s*Generated\b.*\bon\s+(\S+)\s*$/m.exec(text);
...
function main() {
  ...
  writeFileSync(
    ...
    JSON.stringify({ generatedAt: phishingListDate(), domains: securityDomains })
  );
