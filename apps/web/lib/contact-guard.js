 ...
 * without saying so.
 *
 * This route already had a honeypot configured, but no honeypot field was
 * ever rendered, so it could not fire. It could not have caught this
 * anyway: contact-form spam POSTs straight at /api/contact without loading
 * the page, which leaves a hidden field absent from the body rather than
 * filled. The token is the part a request that skipped the page cannot
 * produce.
 *
...
export const contactGuard = secret
  ? createContactGuard({
      secret,
      binding: 'marksyncr:contact',
      brandTerms: ['marksyncr', 'mark syncr'],
